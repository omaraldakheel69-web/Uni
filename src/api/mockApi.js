...
export async function createCompany(payload) { await delay();
 const s = readStore();
...
export async function createService(payload) {
 ...
 // CRITICAL FIX: Ensure price is converted to a NUMBER before storage.
 // It comes in as snake_case string from FormData ('price_per_hour').
 const priceValue = payload.price_per_hour || payload.pricePerHour;
 const pricePerHour = Number(priceValue) || 0; // Use Number() to force conversion

 const item = { 
  id: uid("s"),  ...payload, 
  // Store the standardized, guaranteed NUMBER key
  pricePerHour: pricePerHour,   price_per_hour: undefined, // Ensure the snake_case key is discarded
 };
...
export async function updateService(id, payload) {
 ...
 // FIX 2: Apply the same normalization logic for updates
 let normalizedPayload = { ...payload };
 
 if (normalizedPayload.price_per_hour !== undefined) {
 normalizedPayload.pricePerHour = Number(normalizedPayload.price_per_hour);
  delete normalizedPayload.price_per_hour;
 }

 s.services = s.services.map((c) => 
  c.id === id ? { ...c, ...normalizedPayload } : c
 );
 writeStore(s);
